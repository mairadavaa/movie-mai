        ...
        <div className="Price">
            ...
            <label> Дэлгэц</label>
                <div>
                    <label> A</label>
                    {seatA.map((el, ind) =>
                        <input type="checkbox" className="Seat" onChange={() => setChosenSeat( chosenSeat.map((element, index) => ind===index ? !element : element) )}></input>
                    )}
                </div>
                <div>
                    <label> B</label>
                    {seatB.map((el, ind) =>
                        <input type="checkbox" className="Seat" onChange={() => setChosenSeat( chosenSeat.map((element, index) => ind+12===index ? !element : element) )}></input>
                    )}
                </div>
                <div>
                   <label> C</label>
                    {seatC.map((el, ind) =>
                        <input type="checkbox" className="Seat" onChange={() => setChosenSeat( chosenSeat.map((element, index) => ind+30===index ? !element : element) )}></input>
                    )}
                </div>
                <div>
                <label> D</label>
                    {seatD.map((el, ind) =>
                        <input type="checkbox" className="Seat" onChange={() => setChosenSeat( chosenSeat.map((element, index) => ind+54===index ? !element : element) )}></input>
                    )}
                </div>
            ...
            <div className="Tulbur">
                <div>
                 ...
                 <button className="Button">Төлөх</button>
